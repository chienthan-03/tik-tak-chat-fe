...
function UpdateGroupChatModal({ fetchAgain, setFetchAgain, fetchMessages }) {
  ...
  const { seletedChat, setSelectedChat, user } = ChatState();
  ...
  const handleAddUser = async (user1) => {
    if (seletedChat.users.find((u) => u._id === user1._id)) {
      toast({
    ...
    if (seletedChat.groupAdmin._id !== user._id) {
      toast({
  ...
  const handleRemove = async (user1) => {
    if (seletedChat.groupAdmin._id !== user._id && user1._id !== user._id) {
      toast({
        ...
        <ModalContent>
          ...
          <ModalBody display="flex" flexDir="column" alignItems="center">
            <Box w="100%" display="flex" flexWrap="wrap" pb={3}>
              {seletedChat.users.map((u) => (
                <UserBadgeItem
                  key={u._id}
                  user={u}
                  admin={seletedChat.groupAdmin}
